...
- `player.js` — the player as one embeddable file. Any site adds `<script src="https://cyberia.to/player.js" defer></script>`; it mounts into `[data-cyberia-player]` when the page has one, otherwise adds a fixed bar at the bottom. Position and a deliberate pause persist per origin.
